...
test('recurrence normalization preserves an explicit milking completion action', () => {
  assert.deepEqual(housekeepingData.normalizeRecurrenceRule({ frequency: 'daily', interval: 1, completionAction: 'milk_morning' }), {
    mode: 'fixed_schedule', frequency: 'daily', interval: 1, completionAction: 'milk_morning'
  });
  assert.equal(housekeepingData.normalizeRecurrenceRule({ frequency: 'daily', completionAction: 'guess_from_title' }).completionAction, undefined);
});

test('milking task matching uses record, work date, session, and explicit metadata', () => {
  const configured = { id: 'morning', recordId: 'daisy', dueDate: '2026-08-10', status: 'open', completed: false, recurrenceRule: { frequency: 'daily', interval: 1, completionAction: 'milk_morning' } };
  ...
  assert.deepEqual(housekeepingData.matchingMilkingTasks([configured, titleOnly], yieldEntry).map(task => task.id), ['morning']);
  assert.equal(housekeepingData.matchingMilkingTasks([configured], { ...yieldEntry, session: 'evening' }).length, 0);
});
...
test('a task-linked yield reuses the task cloud identity and existing outbox', () => {
  ...
  after.tasks.push({ id: 'milk-task', recordId: 'daisy', title: 'Barn round', status: 'completed', completed: true, dueDate: '2026-08-10', recurrenceRule: { frequency: 'daily', interval: 1, completionAction: 'milk_morning' }, createdAt: '2026-08-10T00:00:00Z', updatedAt: '2026-08-10T11:00:00Z' });
...
test('offline linked completion queues both changes and reconnect sync succeeds once', async () => {
  ...
  before.tasks.push({ id: 'milk-task', recordId: 'daisy', title: 'Morning round', status: 'open', completed: false, dueDate: '2026-08-10', recurrenceRule: { frequency: 'daily', interval: 1, completionAction: 'milk_morning' }, createdAt: '2026-08-10T00:00:00Z', updatedAt: '2026-08-10T00:00:00Z' });
  ...
  setup.cloud.rows.tasks.push({ id: taskEntity.cloudId, version: 1, record_id: recordEntity.cloudId, title: 'Morning round', status: 'open', due_date: '2026-08-10', recurrence_rule: { frequency: 'daily', interval: 1, completionAction: 'milk_morning' } });
...
test('matching an existing yield ignores deleted entries and does not require title text', () => {
  const task = { recordId: 'daisy', dueDate: '2026-08-10', recurrenceRule: { frequency: 'daily', completionAction: 'milk_evening' } };
