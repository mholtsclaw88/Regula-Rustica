...
test('duplicate structured routines collapse to one active canonical routine', () => {
  ...
  data.routineOccurrences = [{ id: 'old-occurrence', routineId: 'r-old', occurrenceDate: '2026-08-15', status: 'pending' }];
...
test('duplicate system Chore Windows collapse and linked routines are remapped', () => {
  ...
  data.routines = [{ id: 'r1', recordId: 'hens', routineType: 'egg_collection', name: 'Egg Collection', enabled: true, frequency: 'daily', interval: 1, firstDate: '2026-08-16', nextDate: '2026-08-16', choreWindowId: 'morning-duplicate', createdAt: '2026-08-12T00:00:00Z' }];
  const result = stabilization.stabilizeData(data, { now, date: '2026-08-16', makeId: makeIds('today') });
...
test('stale pending occurrence rolls over as missed and only today remains pending', () => {
  const data = baseData();
  data.routines = [{ id: 'r1', recordId: 'cow', routineType: 'milk_morning', name: 'Morning Milking', enabled: true, frequency: 'daily', interval: 1, firstDate: '2026-08-10', nextDate: '2026-08-15', choreWindowId: 'morning', createdAt: '2026-08-10T00:00:00Z' }];
  data.routineOccurrences = [{ id: 'yesterday', routineId: 'r1', occurrenceDate: '2026-08-15', status: 'pending', createdAt: '2026-08-15T00:00:00Z' }];
  const result = stabilization.stabilizeData(data, { now, date: '2026-08-16', makeId: makeIds('today') });
...
test('an every-other-day Routine does not create work on an off day', () => {
  const data = baseData();
  data.routines = [{ id: 'r1', recordId: 'pasture', name: 'Fence check', enabled: true, frequency: 'daily', interval: 2, firstDate: '2026-08-15', nextDate: '2026-08-15', choreWindowId: 'morning', createdAt: '2026-08-15T00:00:00Z' }];
  data.routineOccurrences = [{ id: 'old', routineId: 'r1', occurrenceDate: '2026-08-15', status: 'pending', createdAt: '2026-08-15T00:00:00Z' }];
  stabilization.stabilizeData(data, { now, date: '2026-08-16', makeId: makeIds('unused') });
...
test('stabilization is idempotent for the current day', () => {
  const data = baseData();
  data.routines = [{ id: 'r1', recordId: 'hens', routineType: 'egg_collection', name: 'Egg Collection', enabled: true, frequency: 'daily', interval: 1, firstDate: '2026-08-16', nextDate: '2026-08-16', choreWindowId: 'morning', createdAt: '2026-08-16T00:00:00Z' }];
