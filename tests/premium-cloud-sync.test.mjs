...
const connected = { session: { access_token: 'session' }, homesteadId: 'home', premium: { status: 'active', feature_keys: ['cloud_sync'], ends_at: null } };
...
test('Cloud Sync pauses without an active Premium entitlement but preserves local writes', () => {
  ...
  assert.equal(premiumSyncAvailable({ ...connected, premium: { status: 'active', feature_keys: ['cellarer_assisted_entry'] } }), false);
  assert.equal(premiumSyncAvailable({ ...connected, premium: { status: 'active', feature_keys: ['cloud_sync'], ends_at: '2026-01-01T00:00:00Z' } }, Date.parse('2026-02-01T00:00:00Z')), false);
  assert.equal(premiumSyncAvailable({ ...connected, premium: { status: 'active', feature_keys: ['cloud_sync'], ends_at: '2026-12-01T00:00:00Z' } }, Date.parse('2026-02-01T00:00:00Z')), true);
