-- Return the authoritative Task row when optimistic concurrency detects a
-- conflict so every client can resolve it deterministically.
...
declare
  ...
  window_ref uuid;
  next_status text;
  operation_conflict boolean := false;
begin
  ...
  result := jsonb_build_object(
    'status', case when operation_conflict or current_task.id is null then 'conflict' else 'applied' end,
    'row', current_row
  );
