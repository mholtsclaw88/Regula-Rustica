...
select has_index('public', 'tasks', 'tasks_one_active_routine_per_animal', 'only one active Routine of each type is allowed per Animal');
...
insert into public.records (id,homestead_id,type,name,status,identity,created_by,updated_by) values
  ...
  ('94000000-0000-0000-0000-000000000004',:'homestead_a','animal','Mabel','active','{"purpose":"Dairy"}','91000000-0000-0000-0000-000000000001','91000000-0000-0000-0000-000000000001'),
  ('94000000-0000-0000-0000-000000000005',:'homestead_a','animal','Layers','active','{"purpose":"Eggs"}','91000000-0000-0000-0000-000000000001','91000000-0000-0000-0000-000000000001');

select throws_ok(format($sql$insert into public.tasks (homestead_id,record_id,title,due_date,recurrence_rule)
  values (%L,'94000000-0000-0000-0000-000000000002','Invalid animal','2026-08-10','{"frequency":"daily","interval":1,"routineType":"milk_morning"}')$sql$, :'homestead_a'),
  '23514', 'Routine type does not match the Animal purpose', 'milking Routine rejects a non-dairy Animal');
select throws_ok(format($sql$insert into public.tasks (homestead_id,record_id,title,recurrence_rule)
  values (%L,'94000000-0000-0000-0000-000000000001','Missing date','{"frequency":"daily","interval":1,"routineType":"milk_morning"}')$sql$, :'homestead_a'),
  '23514', 'Routines require a recurring task, eligible Animal, and task date', 'Routine requires a work date');
select throws_ok(format($sql$insert into public.tasks (homestead_id,record_id,title,due_date,recurrence_rule)
  values (%L,'94000000-0000-0000-0000-000000000001','Invalid eggs','2026-08-10','{"frequency":"daily","interval":1,"routineType":"egg_collection"}')$sql$, :'homestead_a'),
  '23514', 'Routine type does not match the Animal purpose', 'Egg Collection rejects a non-laying Animal');

insert into public.tasks (id,homestead_id,record_id,title,due_date,recurrence_rule,created_by,updated_by) values
  ('95000000-0000-0000-0000-000000000001',:'homestead_a','94000000-0000-0000-0000-000000000001','Morning Milking','2026-08-10','{"frequency":"daily","interval":1,"mode":"fixed_schedule","routineType":"milk_morning"}','91000000-0000-0000-0000-000000000001','91000000-0000-0000-0000-000000000001'),
  ('95000000-0000-0000-0000-000000000002',:'homestead_a','94000000-0000-0000-0000-000000000001','Evening Milking','2026-08-10','{"frequency":"daily","interval":1,"mode":"fixed_schedule","routineType":"milk_evening"}','91000000-0000-0000-0000-000000000001','91000000-0000-0000-0000-000000000001'),
  ('95000000-0000-0000-0000-000000000003',:'homestead_a','94000000-0000-0000-0000-000000000004','Assigned milking','2026-08-10','{"frequency":"daily","interval":1,"mode":"fixed_schedule","routineType":"milk_morning"}','91000000-0000-0000-0000-000000000001','91000000-0000-0000-0000-000000000001'),
  ('95000000-0000-0000-0000-000000000004',:'homestead_a','94000000-0000-0000-0000-000000000004','Unassigned milking','2026-08-10','{"frequency":"daily","interval":1,"mode":"fixed_schedule","routineType":"milk_evening"}','91000000-0000-0000-0000-000000000001','91000000-0000-0000-0000-000000000001'),
  ('95000000-0000-0000-0000-000000000005',:'homestead_a','94000000-0000-0000-0000-000000000005','Egg Collection','2026-08-10','{"frequency":"daily","interval":1,"mode":"fixed_schedule","routineType":"egg_collection"}','91000000-0000-0000-0000-000000000001','91000000-0000-0000-0000-000000000001');
select throws_ok(format($sql$insert into public.tasks (homestead_id,record_id,title,due_date,recurrence_rule)
  values (%L,'94000000-0000-0000-0000-000000000005','Duplicate collection','2026-08-10','{"frequency":"daily","interval":1,"routineType":"egg_collection"}')$sql$, :'homestead_a'),
  '23505', null, 'only one active Egg Collection Routine is allowed per Animal');
...
select is((select recurrence_rule ->> 'routineType' from public.tasks where parent_task_id='95000000-0000-0000-0000-000000000001'), 'milk_morning', 'next occurrence keeps the Routine type');
...
select throws_ok($$select public.apply_housekeeping_sync_operation(
  ...
  '23514', 'Yield does not match the linked Routine', 'linked Yield must match the configured session');
select throws_ok($$select public.apply_housekeeping_sync_operation(
  ...
  '23514', 'Yield does not match the linked Routine', 'linked Yield must match the task work date');
select throws_ok($$select public.apply_housekeeping_sync_operation(
...
select throws_ok($$select public.apply_housekeeping_sync_operation(
  'egg-wrong-yield','96000000-0000-0000-0000-000000000001','yield_entries','97000000-0000-0000-0000-000000000005','create',null,'2026-08-10T16:00:00Z',
  '{"record_id":"94000000-0000-0000-0000-000000000005","yield_type":"milk","occurred_at":"2026-08-10T16:00:00Z","session":"other","quantity":8,"unit":"gal","details":{"task_id":"95000000-0000-0000-0000-000000000005"}}')$$,
  '23514', 'Yield does not match the linked Routine', 'Egg Collection rejects a Milk Yield');
select is((public.apply_housekeeping_sync_operation(
  'egg-collection','96000000-0000-0000-0000-000000000001','yield_entries','95000000-0000-0000-0000-000000000005','create',null,'2026-08-10T16:00:00Z',
  '{"record_id":"94000000-0000-0000-0000-000000000005","task_id":"95000000-0000-0000-0000-000000000005","yield_type":"eggs","occurred_at":"2026-08-10T16:00:00Z","session":"other","quantity":8,"unit":"eggs","details":{"task_id":"95000000-0000-0000-0000-000000000005"}}'
) ->> 'status'), 'applied', 'Egg Collection records its Yield through the existing sync RPC');
select is((select status::text from public.tasks where id='95000000-0000-0000-0000-000000000005'), 'completed', 'Egg Yield completes the collection Routine');
select is((select count(*) from public.tasks where parent_task_id='95000000-0000-0000-0000-000000000005'), 1::bigint, 'Egg Collection creates exactly one next occurrence');
select is((select recurrence_rule ->> 'routineType' from public.tasks where parent_task_id='95000000-0000-0000-0000-000000000005'), 'egg_collection', 'next Egg Collection keeps its Routine type');
...
select is((public.apply_housekeeping_sync_operation(
  ...
  '{"record_id":"94000000-0000-0000-0000-000000000004","yield_type":"milk","occurred_at":"2026-08-10T11:00:00Z","session":"morning","quantity":1.5,"unit":"gal","details":{"task_id":"95000000-0000-0000-0000-000000000003"}}') ->> 'status'), 'applied', 'assigned Hand can record Yield and complete the Task');
select throws_ok($$select public.apply_housekeeping_sync_operation(
  ...
  '{"record_id":"94000000-0000-0000-0000-000000000004","yield_type":"milk","occurred_at":"2026-08-10T22:00:00Z","session":"evening","quantity":1.5,"unit":"gal","details":{"task_id":"95000000-0000-0000-0000-000000000004"}}')$$,
