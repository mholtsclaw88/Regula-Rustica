...
export function toCloud(t,r,s,source='manual'){
 ...
 if(t==='chore_windows')return{...c,system_key:r.systemKey||null,name:r.name,display_order:Number(r.displayOrder||0),enabled:r.enabled!==false,daypart:r.daypart||null,start_time:r.startTime||null,end_time:r.endTime||null};
 if(t==='tasks'){const due=r.dueDate||null,available=r.availableFrom||null;return{...c,record_id:cloudId(s,'records',r.recordId),title:r.title,description:r.description||null,status:r.completed?'completed':(r.status||'open'),priority:r.priority||'normal',due_date:due,available_from:due&&available&&available>due?null:available,completed_at:r.completedAt||null,recurrence_rule:r.recurrenceRule||null,parent_task_id:cloudId(s,'tasks',r.parentTaskId),chore_window_id:cloudId(s,'chore_windows',r.choreWindowId),yield_type:r.yieldType||null,suggestion_key:r.suggestionKey||null};}
 ...
 if(t==='calendar_events')return{...c,record_id:cloudId(s,'records',r.recordId),title:r.title,start_date:r.startDate,end_date:r.endDate||r.startDate,all_day:Boolean(r.allDay),start_time:r.allDay?null:(r.startTime||null),end_time:r.allDay?null:(r.endTime||null),location:r.location||null,notes:r.notes||null,recurrence_rule:r.recurrenceRule||null};
...
export function fromCloud(t,r,s){
 ...
 if(t==='chore_windows')return{...c,systemKey:r.system_key,name:r.name,displayOrder:r.display_order,enabled:r.enabled,daypart:r.daypart,startTime:r.start_time?.slice(0,5)||'',endTime:r.end_time?.slice(0,5)||''};
 ...
 if(t==='calendar_events')return{...c,recordId:localId(s,'records',r.record_id),title:r.title,startDate:r.start_date,endDate:r.end_date,allDay:r.all_day,startTime:r.start_time||'',endTime:r.end_time||'',location:r.location||'',notes:r.notes||'',recurrenceRule:r.recurrence_rule||null};
