...
export function toCloud(table, row, state, source = 'manual') {
  ...
  if (table === 'yield_entries') {
    ...
    return { ...common, record_id: cloudId(state, 'records', row.recordId), task_id: taskId, yield_type: row.type, occurred_at: new Date(row.occurredAt).toISOString(), session: row.session || 'other', quantity: Number(row.quantity), unit: row.unit, unusable_quantity: Number(row.unusableQuantity || 0), details: { text: row.details || '', legacy_event_id: cloudId(state, 'chronicle_entries', row.legacyEventId), task_id: taskId } };
  }
