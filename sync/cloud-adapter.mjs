...
export class SupabaseSyncAdapter{
 ...
 async getRow(table,id){if(!DOMAIN_ORDER.includes(table)||!id)return null;const{data,error}=await this.client.from(table).select('*').eq('id',id).maybeSingle();if(error)throw error;return data||null;}
