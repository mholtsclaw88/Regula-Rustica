...
export function rpcForSyncTable(table) {
  const rpc = SYNC_RPC_ROUTES[table];
  if (rpc) return rpc;
...
export class SupabaseSyncAdapter{
 ...
 async apply(operation){const rpc=rpcForSyncTable(operation.table);const{data,error}=await this.client.rpc(rpc,{operation_key:operation.idempotencyKey,client_device_id:operation.deviceId,target_table:operation.table,target_id:operation.rowId,operation_kind:operation.type,expected_version:operation.baseVersion,client_timestamp:operation.clientUpdatedAt,operation_payload:operation.payload});if(error)throw error;return data;}
